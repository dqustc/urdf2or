...
srdf::Model::Group const &GetSRDFGroup(
  ...
  if (it == groups.end()) {
    throw std::runtime_error(
        boost::str(boost::format("Group '%s' does not exist.") % name));
  }
  return it->second;
}

void ExtractSRDFGroup(
    urdf::Model const &urdf, srdf::Model::Group const &group,
  ...
  if (!group.chains_.empty()) {
    ...
    BOOST_FOREACH (boost::tie(base_link_name, tip_link_name), group.chains_) {
      ...
      if (!link_ptr) {
        throw std::runtime_error(
            boost::str(boost::format("Invalid chain in group '%s':"
...
std::pair<OpenRAVE::KinBody::JointType, bool> URDFJointTypeToRaveJointType(
    int type) {
  switch (type) {
    ...
    default:
      throw std::runtime_error(
          boost::str(boost::format("Unkonwn type of joint %d.") % type));
  }
}

void URDFLoader::ParseURDF(
    urdf::Model &model, std::vector<OpenRAVE::KinBody::LinkInfoPtr> &link_infos,
  ...
  while (!link_list.empty()) {
    ...
    if (inertial) {
      ...
      link_info->_vinertiamoments =
          OpenRAVE::Vector(inertial->ixx, inertial->iyy, inertial->izz);
    }
    ...
    BOOST_FOREACH (std::shared_ptr<urdf::Collision> collision,
      ...
      switch (collision->geometry->type) {
        case urdf::Geometry::MESH: {
          ...
          trimesh =
              GetEnv()->ReadTrimeshURI(trimesh, geom_info->_filenamecollision);
          if (trimesh) {
        ...
        case urdf::Geometry::BOX: {
          ...
          geom_info->_vGeomData =
              0.5 * OpenRAVE::Vector(box.dim.x, box.dim.y, box.dim.z);
        ...
        case urdf::Geometry::CYLINDER: {
          ...
          geom_info->_vGeomData =
              OpenRAVE::Vector(cylinder.radius, cylinder.length, 0);
      ...
      switch (visual->geometry->type) {
        case urdf::Geometry::MESH: {
          const urdf::Mesh &mesh =
              dynamic_cast<const urdf::Mesh &>(*visual->geometry);
        ...
        default:
          RAVELOG_WARN(
              "Link[%s]: Only trimeshes are supported for visual geometry.\n",
              link_ptr->name.c_str());
      }
    }
  ...
  BOOST_FOREACH (std::shared_ptr<urdf::Joint> joint_ptr, ordered_joints) {
    ...
    joint_info->_vanchor = URDFVectorToRaveVector(
        joint_ptr->parent_to_joint_origin_transform.position);
    ...
    bool enabled;
    boost::tie(joint_type, enabled) =
        URDFJointTypeToRaveJointType(urdf_joint_type);
    ...
    if (mimic) {
      joint_info->_vmimic[0] =
          boost::make_shared<OpenRAVE::KinBody::MimicInfo>();
      joint_info->_vmimic[0]->_equations[0] =
          boost::str(boost::format("%s*%0.6f+%0.6f") % mimic->joint_name %
                     mimic->multiplier % mimic->offset);
      joint_info->_vmimic[0]->_equations[1] = boost::str(
          boost::format("|%s %0.6f") % mimic->joint_name % mimic->multiplier);
    }

    // Configure joint axis. Add an arbitrary axis if the joint is disabled.
    urdf::Vector3 joint_axis;
    if (enabled) {
      joint_axis = joint_ptr->parent_to_joint_origin_transform.rotation *
                   joint_ptr->axis;
    } else {
...
void URDFLoader::ParseSRDF(
    urdf::Model const &urdf, srdf::Model const &srdf,
  ...
  for (srdf::Model::CollisionPair const &link_pair :
    ...
    if (!link1_info) {
      throw OPENRAVE_EXCEPTION_FORMAT("There is no link named %s.",
                                      link1_name.c_str(), OpenRAVE::ORE_Failed);
    }
    ...
    if (!link2_info) {
      throw OPENRAVE_EXCEPTION_FORMAT("There is no link named %s.",
                                      link2_name.c_str(), OpenRAVE::ORE_Failed);
    }
  ...
  // Create manipulators.
  BOOST_FOREACH (srdf::Model::EndEffector const &end_effector,
    ...
    srdf::Model::Group const &ee_group =
        GetSRDFGroup(groups, end_effector.component_group_);

    if (end_effector.parent_group_.empty()) {
      throw std::runtime_error(
          boost::str(boost::format("End-effector '%s' has no parent group.\n") %
                     end_effector.name_));
    }
    RAVELOG_DEBUG("Loading '%s' manipulator group '%s'.\n",
                  end_effector.name_.c_str(),
                  end_effector.parent_group_.c_str());
    srdf::Model::Group const &manip_group =
        GetSRDFGroup(groups, end_effector.parent_group_);
    ...
    RAVELOG_DEBUG("Manipulator group '%s' contains %d links and %d joints.\n",
                  manip_group.name_.c_str(), manip_links.size(),
                  manip_joints.size());

    if (manip_root_links.size() != 1) {
      throw std::runtime_error(boost::str(
          boost::format(
              "Manipulator '%s' has %d root links; must have exactly one.") %
    ...
    if (it != link_map.end()) {
      ...
      throw std::runtime_error(
          boost::str(boost::format("Unable to find end-effector parent "
                                   " link '%s'.") %
                     end_effector.parent_link_));
    }
    ...
    RAVELOG_DEBUG(
        "Found %d gripper joints for manipulator '%s';"
  ...
  BOOST_FOREACH (srdf::Model::LinkSpheres const &spheres,
    ...
    BOOST_FOREACH (srdf::Model::Sphere const &sphere, spheres.spheres_) {
      ...
      sphere_info->_t.trans =
          collision_transform * OpenRAVE::Vector(sphere.center_x_,
                                                 sphere.center_y_,
                                                 sphere.center_z_);
...
void URDFLoader::ProcessGeometryGroupTagsFromURDF(
  ...
  for (TiXmlElement *link_element = robot_handle.FirstChild("link").Element();
       link_element; link_element = link_element->NextSiblingElement("link")) {
    ...
    if (!link_info) {
      throw std::runtime_error(
          boost::str(boost::format("Unable to find a corresponding link '%s'") %
                     link_name));
    }

    // <geometry_group> element
    const char *geometry_group_name;
    for (TiXmlElement *geometry_group_element =
             link_handle.FirstChild("geometry_group").Element();
         geometry_group_element;
         geometry_group_element =
             geometry_group_element->NextSiblingElement("geometry_group")) {
      ...
      for (TiXmlElement *origin_element =
               geometry_group_handle.FirstChild("origin").Element();
           origin_element;
      ...
      // <geometry> element
      for (TiXmlElement *geometry_element =
               geometry_group_handle.FirstChild("geometry").Element();
           geometry_element;
           geometry_element =
               geometry_element->NextSiblingElement("geometry")) {
        TiXmlHandle geometry_handle = TiXmlHandle(geometry_element);

        // <mesh> element
        for (TiXmlElement *mesh_element =
                 geometry_handle.FirstChild("mesh").Element();
             mesh_element;
...
bool URDFLoader::deprecatedLoad(std::ostream &soutput, std::istream &sinput) {
  RAVELOG_WARN(
      "URDFLoader 'load' command is deprecated. Use 'LoadURI' instead.\n");
